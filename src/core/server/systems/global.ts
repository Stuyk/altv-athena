...
export const Global = {
     ...
     */
    async get<IGlobal>(): Promise<IGlobal> {
        await Global.isReady();
     ...
     */
    async increase(key: string, increaseByValue = 1, startValue = 0) {
        await Global.isReady();
     ...
     */
    async decrease(key: string, decreaseByValue = 1, startValue = Number.MAX_SAFE_INTEGER - 1) {
        await Global.isReady();
